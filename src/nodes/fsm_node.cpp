...
namespace nodes
{
    ...
    void FSMNode::controlLoop()
    {
        ...
        case CORRIDOR: // Drive forward until we detect an intersection
                      ...
                      lidar_around_.right > 0.3))
            {
                ...
                notNormalized_angle = std::remainder(notNormalized_angle, 2.0 * M_PI); // Wrap angle to [-pi, pi]
                target_angle_ = normalize_angle(notNormalized_angle);
                target_lidar_front_ = lidar_around_.front - 0.18; // Set target front distance slightly ahead of current reading to encourage moving forward
        ...
        case INTERSECTION: // Stop and decide which way to turn
            ...
            else
            {
                turn = std::clamp(target_angle_pid_.update(
                                      ...
                                      current_angle_,
                                      0.025),
                                  -ANGLE_SPEED_TRIM, ANGLE_SPEED_TRIM);
                fw_speed = 0.0;
            }
            break;
        ...
        case TURN:
        {
            ...
            if (std::abs(angle_error) < ANGLE_PRECISION)
            {
                ...
                {
                    set_state(CORRIDOR);
                    // target_lidar_front_ = lidar_around_.front - 0.03;
                    fw_speed = 0.1;
                    turn = 0.0;
                }
            }
    ...
    double FSMNode::decide_target_angle()
    {
        if (!is_wall(lidar_around_.left))
        {
            target_lidar_base_ = lidar_around_.left;
            return current_angle_ + M_PI / 2;
        }
        if (!is_wall(lidar_around_.front))
        {
            target_lidar_base_ = lidar_around_.front;
            return current_angle_;
        }
        if (!is_wall(lidar_around_.right))
        {
            target_lidar_base_ = lidar_around_.right;
            return current_angle_ - M_PI / 2;
        }
        target_lidar_base_ = lidar_around_.back;
    ...
    double FSMNode::aruco_target_angle()
    {
        ...
        case FW:
            ...
            RCLCPP_WARN(this->get_logger(), "Forward indicated by marker but front wall detected, defaulting to simple turn logic");
            break;
        case LEFT:
            if (!is_wall(lidar_around_.left))
            {
